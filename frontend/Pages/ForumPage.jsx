...
import {  faMagnifyingGlass, faUsers, faHashtag, faMessage } from '@fortawesome/free-solid-svg-icons'
...
{/* ---------------Trending discussion--------------------------- */}
      <div className='flex justify-between items-center '>
        ...
        <button onClick={createPostModal} className='flex gap-3 items-center mt-16 mb-6 bg-white px-4 py-2 rounded-md'>
      ...
      <div className='flex gap-[5%]'>
        <div className='w-[60%] flex flex-col gap-4'>
          ...
          <div className='border bg-white p-2 rounded-md'>
              <Link to={`/forum/${'Songs and Relatability'}`}><h1 className='text-xl font-semibold josefin text-blue-500 underline'>Songs and Relatability</h1></Link>
              <p className='line-clamp-3 text-black'>I'm older and grew up when music was a way for relaxation. So relatability has never been the primary reason for my listening to an artist or songs. Like how could I find relatability in Bon Jovi's Living on a Prayer as a teen still in school? Or more recently Ed Sheeran's The A Team as I'm not a sex worker or a drug addict. But I'm really moved by the lyrics of these songs every time I hear them.But like the majority of Taylor Swift's fans always cite accessibility and relatability as the main reason for their liking her.How about people here? Are those things important to you?</p>
          </div>
          
          
          {
            forumTopic.slice().reverse().map((topics, index) => (
        ...
        <div className='w-[35%] flex flex-col gap-8'>
          <div className='border p-2'>
              ...
              <div className='flex gap-3 my-2'>
                  ...
                  <Link className='hover:underline'><h1>Swifties</h1></Link>
              </div>

              <div className='flex gap-3 my-2'>
                  <FontAwesomeIcon className='border rounded-full p-1' icon={faUsers}/>
                  <Link className='hover:underline'><h1>Everything Mitsky</h1></Link>
              </div>

              <div className='flex gap-3 my-2'>
                  <FontAwesomeIcon className='border rounded-full p-1' icon={faUsers}/>
                  <Link className='hover:underline'><h1>Live Laugh Lamp</h1></Link>
              </div>

              <div className='flex gap-3 my-2'>
                  <FontAwesomeIcon className='border rounded-full p-1' icon={faUsers}/>
                  <Link className='hover:underline'><h1>Pop Genre Club</h1></Link>
              </div>

              <div className='flex gap-3 my-2'>
                  <FontAwesomeIcon className='border rounded-full p-1' icon={faUsers}/>
                  <Link className='hover:underline'><h1>Music Discussion Group</h1></Link>
              </div>
          </div>
          ...
          <div>
            <div className='border p-2'>
                ...
                <div className='flex gap-3 my-2'>
                    ...
                    <Link className='hover:underline'><h1>The Tortured Poets Album</h1></Link>
                </div>

                <div className='flex gap-3 my-2'>
                    <FontAwesomeIcon className='border rounded-full p-1' icon={faHashtag}/>
                    <Link className='hover:underline'><h1>New Releases</h1></Link>
                </div>

                <div className='flex gap-3 my-2'>
                    <FontAwesomeIcon className='border rounded-full p-1' icon={faHashtag}/>
                    <Link className='hover:underline'><h1>Coachella</h1></Link>
                </div>

                <div className='flex gap-3 my-2'>
                    <FontAwesomeIcon className='border rounded-full p-1' icon={faHashtag}/>
                    <Link className='hover:underline'><h1>Music Video</h1></Link>
                </div>

                <div className='flex gap-3 my-2'>
                    <FontAwesomeIcon className='border rounded-full p-1' icon={faHashtag}/>
                    <Link className='hover:underline'><h1>Tyler The Creator</h1></Link>
                </div>
            </div>
          </div>
        </div>
      ...
      {openModal ? <div className='absolute w-screen h-screen top-0 left-0 backdrop-blur-sm flex justify-center items-center'>
          <div className=' w-[800px] h-[600px] bg-white rounded-md text-black'>
              <div className='flex flex-col px-6 py-3'>
                ...
                <input onChange={event => setTitle(event.target.value)} className='border border-gray-600 rounded-md px-4 py-2' type="text" />
              </div>
              <div className='flex flex-col px-6 py-3'>
